...
void drawMap(char map[MAX_MAP_HEIGHT][MAX_MAP_WIDTH])
{
    ...
    if (startY < 0)
        startY = 0;
    if (endY >= MAX_MAP_HEIGHT)
        endY = MAX_MAP_HEIGHT - 1;
...
void iDraw()
{
    ...
    else if (currentState == STATE_GAME)
    {
        ...
        iShowSpeed(940, 530);
        iShowImage(950, 550, "assets/images/pause_button.bmp");
    ...
    else if (currentState == STATE_SETTINGS)
    {
        ...
        iText(370, 50, "Press 'b' to return ", GLUT_BITMAP_HELVETICA_18);
...
{
    currentState = previousState;  // Go back to wherever we came from
}
...
void iMouse(int button, int state, int mx, int my)
{
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN)
    {
        if (currentState == STATE_MAIN_MENU)
        {
            // ... existing main menu click logic ...
            if (mx >= btnX && mx <= btnX + btnW && my >= btnY + 5 * (btnH + gap) && my <= btnY + 5 * (btnH + gap) + btnH)
            {
                if (!nameEntered || strlen(playerName) == 0)
                {
